...
public:
    inline auto dfs(TreeNode* node, TreeNode* cur, list<TreeNode*>& l) -> bool {
        ...
        if (node->left != nullptr && dfs(node->left, cur, l)) return true;
        if (node->right != nullptr && dfs(node->right, cur, l)) return true;
        l.pop_back();
        return false;
    }
    ...
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        ...
        while (!lP.empty() && !lQ.empty() && lP.front() == lQ.front()) {
            res = lP.front();
            lP.pop_front();
            lQ.pop_front();
        }
...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        ...
        auto res = root;
        while (!pL.empty() && !qL.empty() && pL.front() == qL.front()) {
            res = pL.front();
            pL.pop_front();
            qL.pop_front();
        }
